...
public class BobBuildPluginLoader {
   private Logger       LOGGER =
      Logger.getLogger("BobBuildPluginsLoader");
   private const string PLUGINS_DIRECTORY  = "plugins";
   private const string PLUGIN_INIT_METHOD =
      "initializePlugin";
   private const string PLUGIN_DEPS_METHOD =
      "getDependencies";
   private HashMap<string,
                   AbstractBobBuildPlugin> loadedPlugins =
      new HashMap<string, AbstractBobBuildPlugin>();
   private delegate void ModuleInitFunction(
      BobBuildPluginLoader pluginLoader);


   private delegate string[] GetPluginDependenciesFunction();


   public BobBuildPluginLoader.for_plugins_directory(
      string ? pluginsDirectoryLocation){
      try {
         loadPluginsFromDirectory(
            pluginsDirectoryLocation ?? PLUGINS_DIRECTORY);
      }
   ...
   private void loadPluginsFromDirectory(string pluginsDirectoryLocation)
   throws
   BuildPluginError {
      validateModulesSupported();
      File pluginsDirectory = File.new_for_path(Runtime.resolveRuntimeRelativePath(
                                                   pluginsDirectoryLocation));

      if (!pluginsDirectory.query_exists(null)) {
         throw new BuildPluginError.PLUGINS_FOLDER_MISSING(
            "Plugins folder missing! Skipping further processing.");
      }

      LOGGER.logInfo("Loading plugins from directory: %s",
                     pluginsDirectory.get_path());
      FileEnumerator directoryEnumerator =
         pluginsDirectory.enumerate_children(
      ...
      {
         File pluginFile = pluginsDirectory.get_child(
            fileInfo.get_name());
   ...
   private Module loadPluginModule(string modulePath) throws
   BuildPluginError {
      Module module = Module.open(modulePath, ModuleFlags.BIND_LOCAL);

      if (module == null) {
         throw new BuildPluginError.MODULE_NOT_FOUND_ERROR("Module not found for given path: %s".printf(
                                                              modulePath));
      }
   ...
   private void loadPluginModuleType(Module module) throws BuildPluginError {
      unowned ModuleInitFunction initFunction =
         (ModuleInitFunction)getModuleMethod(module,
                                             PLUGIN_INIT_METHOD);

      initFunction(this);
   }              /* loadPluginModuleType */

   private void *getModuleMethod(Module module,
                                 string methodName) throws BuildPluginError {
      void *methodReference;

      module.symbol(methodName, out methodReference);

      if (methodReference == null) {
         throw new BuildPluginError.MODULE_TYPE_FUNCTION_MISSING_ERROR("'%s' method is missing in plugin module!".printf(
                                                                          methodName));
      }
