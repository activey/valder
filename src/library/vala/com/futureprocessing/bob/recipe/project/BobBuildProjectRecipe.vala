...
public class BobBuildProjectRecipe {
   private List<BobBuildProjectSourceFile> _libSourceFiles =
      new List<BobBuildProjectSourceFile>();
   private List<BobBuildProjectSourceFile> _mainSourceFiles =
      new List<BobBuildProjectSourceFile>();
   public string                           name {
      get;
      set;
   ...
   public void addLibSourceFile(File projectSourceFile){
      _libSourceFiles.append(new BobBuildProjectSourceFile.fromFileSystem(
                                projectSourceFile));
   }              /* addLibSourceFile */

   public void addMainSourceFile(File projectSourceFile){
      _mainSourceFiles.append(new BobBuildProjectSourceFile.fromFileSystem(
                                 projectSourceFile));
   }              /* addMainSourceFile */
