...
public class BobBuildRecipeParser : Object {
   ...
   public static BobBuildRecipe parseFromJSONFile(FileInfo jsonFile) throws
   Error {
      Json.Parser parser = new Json.Parser();
      parser.load_from_file(jsonFile.get_name());
      return new BobBuildRecipeParser().parseFromJSONObject(
   ...
   private void parseProject(Json.Object jsonObject){
      if (keyMissing(jsonObject, MEMBER_PROJECT)) {
         ...
         builder.project(MEMBER_NAME_DEFAULT,
                         MEMBER_SHORTNAME_DEFAULT,
                         MEMBER_VERSION_DEFAULT);
         return;
      }
      Json.Object projectObject =
         jsonObject.get_object_member(MEMBER_PROJECT);
      builder.project(
         getStringMember(projectObject,
                         MEMBER_NAME,
                         MEMBER_NAME_DEFAULT),
         getStringMember(projectObject, MEMBER_SHORTNAME,
                         MEMBER_SHORTNAME_DEFAULT),
         getStringMember(projectObject, MEMBER_VERSION,
                         MEMBER_VERSION_DEFAULT));
   }              /* parseProject */
   ...
   private void parsePlugins(Json.Object jsonObject){
      ...
      Json.Object pluginsObject =
         jsonObject.get_object_member(MEMBER_PLUGINS);
      pluginsObject.foreach_member(parsePluginRecipe);
   }              /* parsePlugins */

   private void parsePluginRecipe(Json.Object jsonObject,
                                  string      pluginName,
                                  Json.Node   recipeFragmentNode){
      LOGGER.logInfo("Parsing plugin recipe: %s", pluginName);
      Json.Object pluginRecipeJson =
         jsonObject.get_object_member(
            pluginName);
      BobBuildPluginRecipe pluginRecipe = new BobBuildPluginRecipe(
         pluginName, new JsonObject.fromJsonObject(pluginRecipeJson));
   ...
   private string getStringMember(Json.Object jsonObject,
                                  string      key,
