...
public class BobBuildRecipeLoader {
   ...
   private Logger       LOGGER       =
      Logger.getLogger("BobBuildRecipeLoader");
   ...
   public BobBuildRecipe ? loadFromJSONFile(
      string jsonFileName)
   throws Error {
   ...
   private FileInfo ? locateRecipeFile(string recipeFileName){
      try {
         ...
         FileEnumerator enumerator    = workDirectory.enumerate_children(
         ...
         LOGGER.logError(
            "An exception occurred while loading json file: ",
            e.message);
      }
      return null;
   }
